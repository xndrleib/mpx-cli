...
async function ConvertWithExportAPI(expectedContentType, url, source, destination, sourceText) {
  ...
  let options = {
    ...
    body: JSON.stringify({
      ...
      fileName: path.basename(destination).split(".")[0],
    }),
  };
...
async function ConvertPDFWithOCRAPI(apiKey, outputType, source, destination) {
  writeProgress(`Converting pdf to ${outputType}\r`);
  ...
  const destinationPath = outputType === "tex" ? `${destination}.zip` : destination;
  try {
    fs.writeFileSync(destinationPath, bufferResponseBody);
  } catch (err) {
    console.log(`Could not write to destination path: ${destination}`);
    return process.exit(1);
  }
...
async function ConvertPDFWithSnipAPI(authToken, outputType, source, destination) {
  ...
  if (outputType === "docx" || outputType === "html" || outputType === "tex") {
    const mmdURL = `https://api.mathpix.com/v1/pdfs/${pdfID}/mmd`;
    const mmdRes = await fetch(mmdURL, { method: "GET", headers: headers });
    if (mmdRes.status !== 200) {
      console.log(`Unexpected error, please submit a bug report at: ${pkg.repository.issues}`);
      return process.exit(1);
    }
    ...
    if (outputType === "docx") {
      await MarkdownToDocx(mmd, destination, true);
      return;
    }

    if (outputType === "tex") {
      await MarkdownToTex(mmd, destination, true);
      return;
    }
...
async function PDFToTEX(source, destination) {
  await ConvertPDF("tex", source, destination);
}
...
async function ConvertImageWithSnipAPI(authToken, outputType, source, destination, htmlPDF) {
  writeProgress(`Converting image to ${outputType}\r`);
...
async function ImageToTEX(source, destination) {
  await ConvertImage("tex", source, destination);
}
